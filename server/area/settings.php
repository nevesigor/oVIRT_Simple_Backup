    ...
    if ( ! empty( $automatedinprocess ) ) {
        ...
        if ( $savestep == 1 || empty( $settings ) ) {
            ...
            if ( ! empty( $savestepstart ) ) {
                ...
                $withoutmemory       = varcheck( "withoutmemory", 0, "FILTER_VALIDATE_INT", 0 );
            ...
            fwrite( $configfile, '"withoutmemory" => "' . $withoutmemory . '",' . "\n" );
            fwrite( $configfile, ');' . "\n" );
            fclose( $configfile );
            ...
            $rowdata = array(
                ...
                array(
                    "text" => sb_input( array(
                        ...
                        'name'  => 'withoutmemory',
                        'list'  => array(
                            array( 'id' => '0', 'name' => 'With Memory', ),
                            array( 'id' => '1', 'name' => 'Without Memory', ),
                        ),
                        'value' => $settings['withoutmemory'],
                    ) ),
                ),
                array(
                    "text" => "",
